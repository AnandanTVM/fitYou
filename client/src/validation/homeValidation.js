...
const  link = /^(?:https?:\/\/)?(?:m\.|www\.)?(?:youtu\.be\/|youtube\.com\/(?:embed\/|v\/|watch\?v=|watch\?.+&v=))((\w|-){11})(?:\S+)?$/;
...
export const trainerSchema = yup.object().shape({
    ...
    password: yup.string().min(5).max(16).matches(passwordRule, "please Create a stronger password").required("Required"),
    cpassword: yup.string().oneOf([yup.ref("password"), null], "Password must match").required("Required"),  
    link:yup.string().matches(link, "Please paste a valid youtube link here").required("Required")
