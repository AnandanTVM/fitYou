...
import { getActiveTrainerInfo,unBlockTrainer,blockTrainer } from '../../axios/serives/AdminServices';
...
async function Block(id) {
  ...
  {
    name:"status",
    selector:(row)=>{return(<div style={{color:"green"}}>  {row.status}</div>)},
  },
  {
    name:"Block/Unblock",
    selector:(row)=>{return(<div> {row.block ? <button className="btn btn-danger" onClick={() =>unBlock(row._id)} >Un Block</button> 
    : <button className="btn btn-danger"  onClick={() =>Block(row._id)} >Block</button>}</div>)},
  },
  {
    name:"View More",
